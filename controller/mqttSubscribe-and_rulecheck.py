...
def ruleCheck(value, sensor, client,states):
    ...
    if sensor.name == "greenify/tempSensor":
        if float(value)  > 40:
            ...
            sensor.updateSensorState(states[2],client)
        # For Potentiometer
    if sensor.name == "greenify/potentiometer":
        if float(value)  > 2000:
            print("Alert! PH Level is too high.")
            #sensor.updateSensorState(states[0],client)
        elif float(value) > 1500 and float(value) < 2000:
            sensor.updateSensorState(states[1],client)
        elif float(value)  < 2000:
            sensor.updateSensorState(states[2],client)
    return
...
def run():
    ...
    t3 = Sensor("greenify/potentiometer", [{"low":"start"}, {"optimal":"stop"}, {"high":"start"}], 0, "greenify/waterironizer")
